...
Vector::Vector() { m_x = m_y = m_z = 0; }

Vector::Vector(const Number x0, const Number y0, const Number z0) : m_x(x0), m_y(y0), m_z(z0){};

Vector::Vector(const double x0, const double y0, const double z0) : m_x(x0), m_y(y0), m_z(z0){};
...
Vector Vector::operator*(const double &doubleValue) const { return *this * Number(doubleValue); }
...
Vector Vector::getNormalized() const { return (*this) / this->length(); };
...
const Number &Vector::getX() const { return m_x; }

void Vector::setX(const Number &x) { m_x = x; }

const Number &Vector::getY() const { return m_y; }

void Vector::setY(const Number &y) { m_y = y; }

const Number &Vector::getZ() const { return m_z; }

void Vector::setZ(const Number &z) { m_z = z; };

Vector operator*(const Number &s, const Vector &v) {
    return Vector(v.m_x * s, v.m_y * s, v.m_z * s);
}

Vector operator*(const double &s, const Vector &v) { return v * Number(s); }