...
void Scene::addObject(std::shared_ptr<Object> object) { objects.emplace_back(std::move(object)); }

void Scene::addLight(std::shared_ptr<Light> light) { lights.emplace_back(std::move(light)); }
...
Color Scene::trace(Ray &r, int d) {
    ...
    if (hit.hittingTime > 0.0f) {
        ...
        /* Azért nem lehet itt, mert akkor nem tudjuk, a refractban, hogy belülről vagy kívülről
        megyünk. Ezért kell a Rough/reflective blokkon belülre rakni. if (hit.normal * r.direction >
        0){ hit.normal = hit.hittedObject->getNormalAtPoint(hit.intersectPoint).getNormalized();
        ...
        if (hit.hittedObject->material->isRough()) {
            ...
            for (const auto &light : lights) {
                ...
                if (shadowHit.hittingTime.value < 0.0f ||
                    (shadowHit.hittingTime * shadowRay.direction).length() >=
                        light->getDistance(hit.intersectPoint)) {
                    ...
                    Vector h = (shadowRay.direction.getNormalized() - r.direction.getNormalized())
                                   .getNormalized();
            ...
            if (hit.hittedObject->material->isReflective()) {
                Vector reflectedDir =
                    hit.hittedObject->material->reflect(r.direction.getNormalized(), hit.normal)
                        .getNormalized();
                Color F =
                    hit.hittedObject->material->Fresnel(r.direction.getNormalized(), hit.normal);
                Ray reflectedRay(hit.intersectPoint + 0.01f * reflectedDir,
                                 reflectedDir.getNormalized());
                ret = ret + F * trace(reflectedRay, d + 1);
            }
            if (hit.hittedObject->material->isRefractive()) {
                Vector refractedDir;
                if (hit.hittedObject->material->refract(r.direction.getNormalized(), hit.normal,
                                                        refractedDir)) {
                    Color F = hit.hittedObject->material->Fresnel(r.direction.getNormalized(),
                                                                  hit.normal);
...
void Scene::build() {
    ...
    Material *diagonal = new TableMaterial(Number(0), Number(5), Color(0.0f, 0.0f, 0.0f),
                                           Color(1.0f, 0.3f, 0.3f) / 2, Color(0.3, 0.3, 0.3), ROUGH,
                                           diagonalKD, diagonalKD);

    Material *squared = new TableMaterial(Number(0), Number(5), Color(0.0f, 0.0f, 0.0f),
                                          Color(1.0f, 0.3f, 0.3f) / 2, Color(0.3, 0.3, 0.3), ROUGH,
                                          squaredKD, squaredKD);
    ...
    Material *gold =
        new Material(Number(0), Number(0), goldF0, Color(0, 0, 0), Color(0, 0, 0), REFLECTIVE);
    ...
    std::shared_ptr<InfiniteCylinder> roomCylinder =
        std::make_shared<InfiniteCylinder>(gold, Number(30));
    ...
    Material *glass =
        new Material(Number(1.5f), Number(0), glassF0, Color(0, 0, 0), Color(0, 0, 0), REFRACTIVE);

    std::shared_ptr<InfiniteCylinder> glassCylinder =
        std::make_shared<InfiniteCylinder>(glass, Number(3));
    ...
    Material *silver =
        new Material(Number(0), Number(0), silverF0, Color(0, 0, 0), Color(0, 0, 0), REFLECTIVE);

    std::shared_ptr<InfiniteCylinder> silverCylinder =
        std::make_shared<InfiniteCylinder>(silver, Number(4));
    ...
    Material *cylinderMaterial = new TableMaterial(
        Number(0), Number(5), Color(0.0f, 0.0f, 0.0f), Color(1.0f, 0.3f, 0.3f) / 2,
        Color(0.3, 0.3, 0.3), ROUGH, cylinderKD, cylinderKD);

    std::shared_ptr<InfiniteCylinder> texturedCylinder =
        std::make_shared<InfiniteCylinder>(cylinderMaterial, Number(3));
    ...
    /// Point lights
    std::shared_ptr<PointLight> blueLight =
        std::make_shared<PointLight>(Vector(10, 10, 10), Color(0, 0, 50));
    addLight(std::move(blueLight));

    std::shared_ptr<PointLight> redLight =
        std::make_shared<PointLight>(Vector(-5, 20, 15), Color(50, 0, 0));
...
void Scene::smoothTask(Scene *scene, Color *targetImage) {
    ...
    while (scene->nextPixelNumber < scene->screenWidth * scene->screenWidth) {
        ...
        if (scene->needSmoothing(xPos, yPos))
            ...
            targetImage[yPos * scene->screenWidth + xPos] =
                scene->image[yPos * scene->screenWidth + xPos];
