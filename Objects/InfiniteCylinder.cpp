...
Vector InfiniteCylinder::getNormalAtPoint( Vector &intersectPoint ) {
    ...
    return intersectPoint - convertToWorldsCoords( Vector( 0, ownIntersectPoint.getY(), 0 ));
}
...
Number InfiniteCylinder::intersect( Ray &r, RayHit &hit ) {
    ...
    Number c = source2D * source2D - pow( radius, 2 );
    if( a != 0.0f ){
        ...
        if( determinant == 0.0f ){
            ...
            Number x1 = ( -b + sqrt( determinant )) / ( 2.0f * a );
            Number x2 = ( -b - sqrt( determinant )) / ( 2.0f * a );
