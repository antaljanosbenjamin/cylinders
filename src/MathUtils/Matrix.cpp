...
void Matrix::clear() {
    matrix[0][0] = matrix[0][1] = matrix[0][2] = matrix[0][3] = matrix[1][0] = matrix[1][1] =
        matrix[1][2] = matrix[1][3] = matrix[2][0] = matrix[2][1] = matrix[2][2] = matrix[2][3] =
            matrix[3][0] = matrix[3][1] = matrix[3][2] = matrix[3][3] = Number();
}
...
Vector operator*(const Vector v, const Matrix &mat) {
    Number Xh = mat.matrix[0][0] * v.m_x + mat.matrix[1][0] * v.m_y + mat.matrix[2][0] * v.m_z +
                mat.matrix[3][0];
    Number Yh = mat.matrix[0][1] * v.m_x + mat.matrix[1][1] * v.m_y + mat.matrix[2][1] * v.m_z +
                mat.matrix[3][1];
    Number Zh = mat.matrix[0][2] * v.m_x + mat.matrix[1][2] * v.m_y + mat.matrix[2][2] * v.m_z +
                mat.matrix[3][2];
    Number h = mat.matrix[0][3] * v.m_x + mat.matrix[1][3] * v.m_y + mat.matrix[2][3] * v.m_z +
               mat.matrix[3][3];
    return Vector(Xh / h, Yh / h, Zh / h);
};