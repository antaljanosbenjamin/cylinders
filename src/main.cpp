...
class FullScreenTexturedQuad {
  ...
  public:
    void Create(Color image[windowWidth * windowHeight]) {
        ...
        // vertex coordinates: vbo[0] -> Attrib Array 0 -> vertexPosition of the
        // vertex shader
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(vertexCoords), vertexCoords,
                     GL_STATIC_DRAW); // copy to that part of the memory which is
                                      // not modified
        ...
        m_image = image;
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, windowWidth, windowHeight, 0, GL_RGB, GL_FLOAT,
                     image); // To GPU
    ...
    void Draw() {
        glBindVertexArray(vao); // make the vao and its vbos active playing the role
                                // of the data source
        ...
        if (location >= 0) {
            ...
            glBindTexture(GL_TEXTURE_2D,
                          textureId); // connect the texture to the sampler
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, windowWidth, windowHeight, 0, GL_RGB, GL_FLOAT,
                         m_image); // To GPU
        }
...
Color background[windowWidth * windowHeight]; // The image, which stores the ray
                                              // tracing result
...
void onInitialization() {
    ...
    for (int x = 0; x < windowWidth; x++) {
        for (int y = 0; y < windowHeight; y++) {
            background[y * windowWidth + x] =
                Color((float)x / windowWidth, (float)y / windowHeight, 0);
        }
    }
    ...
    Camera cam =
        Camera(eye, lookat, Vector(0, 1, -1), Number((int)windowWidth), Number((int)windowHeight));
    ...
    glBindFragDataLocation(shaderProgram, 0,
                           "fragmentColor"); // fragmentColor goes to the frame buffer memory
...
void onMouse(int button, int state, int pX, int pY) {
    if (button == GLUT_LEFT_BUTTON && state == GLUT_DOWN) { // GLUT_LEFT_BUTTON / GLUT_RIGHT_BUTTON
                                                            // and GLUT_DOWN / GLUT_UP
    }
}
