...
Object::Object(Material *material_init)
    : transform(), inverseTransform(), scalem(), rotate(), shift(), inversScale(), inverseRotate(),
      inverseShift(), material(material_init) {
    clearTransform();
}
...
Ray Object::convertRayToObjectCoords(Ray &r) {
    return Ray(convertToObjectCoords(r.source),
               convertToObjectCoords(r.source + r.direction) - convertToObjectCoords(r.source));
}
...
Vector Object::convertToObjectCoords(Vector v) { return v * inverseTransform; }

Vector Object::convertToWorldsCoords(Vector v) { return v * transform; }
...
Color Object::getKS() { return material->getKS(); }

Number Object::getShininess() { return material->getShineness(); }